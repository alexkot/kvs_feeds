...
$categories_filter=trim($_GET['categories']); //custom
...
if ($_GET['action']=='get_deleted' || $_GET['action']=='get_deleted_ids' || $_GET['action']=='get_deleted_urls')
{
    ...
    if (in_array('old_db',$csv_columns)){
        ...
        foreach ($data as $item)
        {
            ...
            if(isset($_GET['promo']) && $_GET['promo']==='1102') $url=str_replace("//www.","//",$url);
    ...
    if (in_array($feed_format,array('csv','kvs')))
    {
        ...
        foreach ($data as $k=>$video) {
          if(isset($_GET['promo']) && $_GET['promo']==='1102') $data[$k]['website_link']=str_replace("//direct.","//",$video['website_link']);
          $data[$k]['website_link'] = str_replace("direct.","www.",$video['website_link']);
